import type { ExportedCustomPath, Point } from "../types";
...
export class CustomPathManager {
    ...
    return JSON.stringify(
      {
        ...
        speed: this.speed,
        points: this.points
      } satisfies ExportedCustomPath,
      null,
      2
    );
  }

  importPath(jsonText: string): void {
    const parsed = JSON.parse(jsonText) as Partial<ExportedCustomPath>;
  ...
  getPointAtDistance(distance: number): Point | null {
    const pts = this.getTraversalPoints();
